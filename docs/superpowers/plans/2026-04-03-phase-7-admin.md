# Phase 7 — Admin : Activation comptes coaches

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Interface admin permettant à Johnny d'activer ou désactiver les comptes coaches. Vue simple : liste des coaches avec statut, bouton toggle actif/en attente.

**Architecture:** Écran admin protégé par le rôle `admin` (déjà géré par le root layout). Requête directe Supabase sur `coaches` + `profiles`. Pas de logique complexe — juste lecture et update de `coaches.status`.

**Tech Stack:** Supabase JS v2, Expo Router, TypeScript

---

## Task 1 : RLS policy admin + écran admin

**Files:**
- Create: `supabase/migrations/002_admin_policies.sql`
- Modify: `app/(admin)/index.tsx`

- [ ] **Step 1 : Créer `supabase/migrations/002_admin_policies.sql`**
...
Le développeur doit appliquer cette migration via le MCP Supabase ou le SQL Editor.

- [ ] **Step 3 : Réécrire `app/(admin)/index.tsx`**
...
```bash
git add supabase/migrations/002_admin_policies.sql app/(admin)/index.tsx
git commit -m "feat: admin screen to activate/deactivate coach accounts"
```

---

## Résultat de la Phase 7

- Interface admin listant coaches actifs et en attente
- Toggle activer/désactiver avec confirmation
- Policies RLS admin en place

**Phase 7 = dernière phase. App V1 complète.**
